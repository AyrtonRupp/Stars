...
document.addEventListener("click", function iniciarMusica() {
    ...
    document.removeEventListener("click", iniciarMusica); // Remueve el evento tras el primer clic
});

document.addEventListener("mousemove", function iniciarMusica() {
    const audio = document.getElementById("background-music");
    audio.play().then(() => {
        console.log("Audio iniciado automáticamente con el movimiento del mouse.");
        document.removeEventListener("mousemove", iniciarMusica);
    }).catch(error => {
